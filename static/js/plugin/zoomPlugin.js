 ...
 *
 * This plugin is modified from the original at https://github.com/chartjs/chartjs-plugin-zoom to fix
 * a bug with hammer pinch event listeners not being registered if the chart's initial state has
 * pinch gestures disabled
 */
import Hammer from './hammer';
import { callback, each, getRelativePosition, sign, valueOrDefault } from 'chart.js/helpers';
...
function zoomDelta(scale, zoom, center) {
    ...
    const minPercent = Math.max(
        0,
        Math.min(1, (scale.getValueForPixel(centerPoint) - scale.min) / range || 0),
    );
...
function panNumericalScale(scale, delta, limits, canZoom = false) {
    ...
    const { min: minLimit = -Infinity, max: maxLimit = Infinity } =
...
function startHammer(chart, options) {
    ...
    // Updated from library, always register pinch listener if it exists in the options
    if (zoomOptions && zoomOptions.pinch) {
        mc.add(new Hammer.Pinch({ pointers: 2, threshold: 0 }));
